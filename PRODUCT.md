...
- **Analytics:** Python under `services/crawley` (`src/crawley`) — semi-autonomous daemons (ASX scan, Gmail ingest, …) with private worker storage that **publish** results; a thin FastAPI **JSON API** for jobs + presentation reads; Google OAuth and LLM stay here.
...
| Pivot Sprint 32 | Operator uses `crawley-ui` ASX pack against analytics | **Met** (pack + proxy) |
...
**Sprint 35:** Jinja/HTMX product UI **deleted**; Calendar + lite modules **quarantined** under `services/crawley/src/crawley/_quarantine/` (not in registry).  
...
- **Calendar** + lite modules **quarantined** (`services/crawley/src/crawley/_quarantine/`); return path = daemon + pack
